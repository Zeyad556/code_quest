    ...
    return Scaffold(
      ...
      body: BlocBuilder<PythonCourseCubit, PythonCourseState>(
        builder: (context, state) {
          ...
          } else if (state is PythonCourseLoaded) {
            return ListView.builder(
              itemCount: state.courses.length,
              itemBuilder: (context, index) {
                return buildCourseList(state.courses[index]);
              },
            );
          } else if (state is PythonCourseFailure) {
            return Center(
              child: Text('Failed to load courses: ${state.error}'),
            );
          }
          return Container(); // Placeholder for other state
        },
      ),
    );
  }
